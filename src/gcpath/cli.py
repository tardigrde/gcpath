...
def main(
    use_asset_api: bool = typer.Option(
        True, "--use-asset-api/--no-use-asset-api", "-u/-U",
...
@app.command()
def ls(
    organizations: Annotated[Optional[List[str]], typer.Argument(help="Organization display names to filter (optional)")] = None,
    long: bool = typer.Option(False, "--long", "-l", help="Show resource names along with paths"),
    recursive: bool = typer.Option(True, "--recursive/--no-recursive", "-R/-r", help="List resources recursively"),
):
    """
    ...
    try:
        hierarchy = Hierarchy.load(
            ...
            via_resource_manager=not state["use_asset_api"]
        )
        ...
        if not hierarchy.organizations and not hierarchy.projects:
            ...
            rprint(f"[yellow]No organizations or projects found accessible to your account{account_msg}.[/yellow]")
            if not account_msg:
                rprint("[dim]Hint: You might not have access to any organizations. Projects without organizations are shown with //_ prefix.[/dim]")
            return
        ...
        for org in hierarchy.organizations:
            if recursive:
                # Add org itself
                items.append((f"//{path_escape(org.organization.display_name)}", org.organization.name))
...
def tree(
    organizations: Annotated[Optional[List[str]], typer.Argument(help="Organization display names to filter (optional)")] = None,
    level: int = typer.Option(None, "--level", "-L", help="Max display depth of the tree"),
    show_ids: bool = typer.Option(False, "--ids", "-i", help="Show resource names in the tree"),
):
    """
...
def get_resource_name(
    paths: Annotated[List[str], typer.Argument(help="Paths to resolve, e.g. //example.com/folder")],
    id_only: bool = typer.Option(False, "--id", help="Print only the resource ID number"),
):
    """
...
def get_path_command(
    resource_names: Annotated[List[str], typer.Argument(help="Resource names to resolve, e.g. folders/123")],
):
    """
